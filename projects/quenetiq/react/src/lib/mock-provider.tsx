...
function buildMockClient(
	...
	const mockMiddleware: GraphqlMiddleware = async (request: GraphqlRequestContext) => {
		...
		if (strict) {
			throw new Error(
				`[@quenetiq/react] No mock found for ${request.type}: ${request.query.slice(0, 80)}` +
...
export function MockedProvider({
	...
	cacheConfig,
	cache: cacheProp,
}: MockedProviderProps): ReactNode {
	const { client, cache } = useMemo(() => {
		const c = cacheProp ?? (cacheConfig ? createCache(cacheConfig) : createCache());
		const schemaMock = schema ? createSchemaMock({ schema, typeMocks }) : null;
		const cl = buildMockClient(mocks, schemaMock, addTypename, strict, c);
		return { client: cl, cache: c };
	}, [mocks, schema, typeMocks, addTypename, strict, cacheConfig, cacheProp]);
