...
All query and mutation composables support an `AbortSignal`. Pass `signal` in options to cancel in-flight requests:
...
const { data } = useQuery(gql`query Todos { todos { id title } }`, {
  signal: controller.signal,
});

// Cancel on button click or component unmount
...
Directly write to the normalized cache without executing a network request:

```ts
import { useWriteQuery, useWriteFragment, gql } from '@quenetiq/vue';

const writeQuery = useWriteQuery();
const writeFragment = useWriteFragment();

// Write a full query result to cache
writeQuery(GET_TODOS, { todos: [{ id: '1', title: 'Cached todo' }] });

// Write a single fragment entity
writeFragment(TODO_FIELDS, { __typename: 'Todo', id: '1', title: 'Updated' });
```

Use `mutationCachePolicy` to control how mutation results merge into cache:
...
const { mutate } = useMutation(ADD_TODO, {
  mutationCachePolicy: 'merge', // 'overwrite' | 'merge' | 'none'
});
```

## Partial / Resumable Queries

Fetch large result sets incrementally. `usePartialQuery` returns paginated chunks that you accumulate:

```vue
<script setup>
import { usePartialQuery, gql } from '@quenetiq/vue';

const ALL_USERS = gql`
  query AllUsers { users { id name email } }
`;

const { data, loading, fetchNext, hasMore } = usePartialQuery(ALL_USERS, {
  pageSize: 50,
});
</script>
...
<template>
  ...
  <button v-if="hasMore" @click="fetchNext" :disabled="loading">Load more</button>
</template>
```
