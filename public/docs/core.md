...
```typescript
const posts = this.graphql.query(gql`
	query Posts {
		posts {
			id
			title
...
const mutation = this.graphql.mutate(gql`
...
```typescript
const createPost = this.graphql.mutate(gql`
	mutation CreatePost($input: PostInput!) {
		createPost(input: $input) {
			id
			title
...
const request = buildRequest(gql`
	query {
		ping
	}
`);
...
const mutationResult = mutate(gql`
...
export const appConfig: ApplicationConfig = {
	providers: [provideGraphql({ endpoint: '/graphql' })],
};
```
...
The following feature pages provide detailed documentation for specific `@quenetiq/core` capabilities:

| Feature                                         | Description                                                                                  |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------- |
| [Cache Writes](cache-writes)                    | Injectable helpers for direct cache manipulation (`injectWriteQuery`, `injectWriteFragment`) |
| [Glob Cache Invalidation](glob-invalidation)    | Glob-pattern cache invalidation (`injectGlobInvalidation`)                                   |
| [Mutation Batching](mutation-batching)          | Batching multiple mutations into a single network tick (`MutationBatchService`)              |
| [Conditional Queries](skip-query)               | Conditionally skip query execution (`skipQuery`)                                             |
| [Null Detection](null-detection)                | Response null detection service and overlay (`NullCheckerService`)                           |
| [Response Diff Logging](response-diff)          | Log changes between consecutive responses (`responseDiffLogging`)                            |
| [Endpoint Decorator & Resolver](use-endpoint)   | Named endpoint decorator and runtime URL resolvers                                           |
| [Endpoint Discovery](endpoint-discovery)        | Auto-validate endpoint YAML against running schemas                                          |
| [Endpoint Mocking](endpoint-mock)               | Schema-aware automatic mock middleware for endpoints                                         |
| [Multi-Endpoint Subscriptions](subscribe-multi) | WebSocket subscriptions with named endpoint resolution                                       |
| [Infer Types](infer-types)                      | TypeScript type inference utilities for GraphQL documents                                    |
...
| `skipQuery(document, endpoint?, variables?, options?)` | function  | Conditional query — only executes when `options.skip` is falsy. Returns `SkipQueryHandle` with signals.                                         |
| `SkipQueryHandle<T>`                                   | interface | Signals + Observable for conditional queries: `data`, `error`, `loading`, `status`, `result$`, `enabled`, `refetch`.                            |
...
| Member                     | Type      | Description                                                                                  |
| -------------------------- | --------- | -------------------------------------------------------------------------------------------- |
| `injectWriteQuery(di?)`    | function  | Returns a `(options, data) => void` function for writing full entity data into the cache.    |
| `injectWriteFragment(di?)` | function  | Returns a `(options, value) => void` function for writing a single field of a cached entity. |
| `WriteQueryOptions`        | interface | Options for `injectWriteQuery`: `__typename`, `id?`.                                         |
| `WriteFragmentOptions`     | interface | Options for `injectWriteFragment`: `__typename`, `id?`, `field`.                             |

### Glob Invalidation

| Member                        | Type     | Description                                                                                                                                                     |
| ----------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `injectGlobInvalidation(di?)` | function | Returns a `(glob: string) => number` function that invalidates cache entries matching the glob pattern. `*` matches single segment, `**` matches multi-segment. |

### Mutation Batching

| Member                                                               | Type      | Description                                                                      |
| -------------------------------------------------------------------- | --------- | -------------------------------------------------------------------------------- |
| `MutationBatchService`                                               | class     | Injectable service for batching mutations within a time window.                  |
| `MutationBatchService.add(document, variables?, endpoint?, config?)` | method    | Adds a mutation to the batch. Returns `Observable<GraphQLResult<T>>`.            |
| `MutationBatchConfig`                                                | interface | Config: `batchWindowMs` (50), `maxBatchSize` (10).                               |
| `MutationBatchItem<TResponse>`                                       | interface | Internal batch item: `document`, `variables?`, `endpoint?`, `resolve`, `reject`. |

### Null Detection

| Member                                                   | Type     | Description                                                                                              |
| -------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------- |
| `NullCheckerService`                                     | class    | Injectable service for recursively walking response data and reporting nulls via `NullDetectionService`. |
...
| Member                         | Type      | Description                                                                                                                         |
| ------------------------------ | --------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `responseDiffLogging(config?)` | function  | Creates a diff middleware that logs changes between consecutive responses for the same query. Returns `(entry: DiffEntry) => void`. |
| `DiffEntry`                    | interface | `timestamp`, `query`, `changedFields`, `previousData`, `newData`.                                                                   |
...
| Member                                    | Type      | Description                                                                                            |
| ----------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------ |
| `EndpointDiscoveryService`                | class     | Injectable service that probes all endpoints in YAML config for accessibility and schema availability. |
| `EndpointDiscoveryService.discover(yaml)` | method    | Returns `Promise<DiscoveryResult[]>`.                                                                  |
| `DiscoveryResult`                         | interface | `routeName`, `url`, `accessible`, `hasSchema`, `sdlPreview?`, `error?`.                                |

### Endpoint Mocking

| Member                            | Type      | Description                                                                    |
| --------------------------------- | --------- | ------------------------------------------------------------------------------ |
| `endpointMockMiddleware(config?)` | function  | Schema-aware mock middleware. Intercepts requests and returns typed stub data. |
| `EndpointMockConfig`              | interface | `schema?`, `mocks?`, `delay?`, `passthrough?`.                                 |
| `MockFieldResolver`               | type      | `(typeName, fieldName) => unknown`.                                            |

### Multi-Endpoint Subscriptions

| Member                                            | Type     | Description                                                                            |
| ------------------------------------------------- | -------- | -------------------------------------------------------------------------------------- |
| `resolveSubscriptionUrl(endpointName)`            | function | Resolves WebSocket URL from a named endpoint. Converts HTTP → WS, HTTPS → WSS.         |
| `subscribeTo(endpointName, document, variables?)` | function | Creates a subscription `Observable` bound to a named endpoint. Resolves WS URL via DI. |

### Infer Types

| Member                             | Type | Description                                                        |
| ---------------------------------- | ---- | ------------------------------------------------------------------ |
| `InferResultData<T>`               | type | Extracts data shape from `GraphQLResult<T>`                        |
| `InferResultError<T>`              | type | Extracts error type from `GraphQLResult<T>`                        |
| `InferSuccessData<T>`              | type | Extracts data from the success variant                             |
| `InferResponse<T>`                 | type | Infers response type from `TypedDocumentNode` / `TypedQueryString` |
| `InferVariables<T>`                | type | Infers variables type from a document                              |
| `InferDocument<T>`                 | type | Extracts the document type                                         |
| `InferQueryData<T, TVars>`         | type | Infers query result data for a document                            |
| `InferQuerySignals<T, TVars>`      | type | Infers signal-based query return type                              |
| `InferEndpointNames<Yaml>`         | type | Extracts endpoint name keys from `EndpointsYaml`                   |
| `InferEndpointRoute<Yaml, Name>`   | type | Infers route config for a specific endpoint name                   |
| `InferEndpointUrl<Yaml, Name>`     | type | Infers URL string type for an endpoint                             |
| `InferEndpointHeaders<Yaml, Name>` | type | Infers headers type for an endpoint                                |
| `InferMiddleware<Mw>`              | type | Infers middleware type from a middleware array                     |
| `InferMiddlewareRequest<Mw>`       | type | Infers request type accepted by middleware                         |
| `InferObservableData<T>`           | type | Extracts data type from an Observable                              |
