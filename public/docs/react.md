...
All query and mutation hooks support an `AbortSignal`. Pass `signal` in options to cancel in-flight requests:
...
function CancellableQuery() {
  ...
  return <button onClick={() => controller.abort()}>Cancel</button>;
}
```
...
```tsx
import { useWriteQuery, useWriteFragment, gql } from '@quenetiq/react';
...
```tsx
import { usePartialQuery, gql } from '@quenetiq/react';
...
function UserList() {
  ...
  return (
    <>
      ...
      {hasMore && <button onClick={fetchNext} disabled={loading}>Load more</button>}
    </>
  );
}
```
