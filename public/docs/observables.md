...
package: "@quenetiq/observables"
---

# @quenetiq/observables

RxJS operators that bridge `@quenetiq/cache` with reactive streams. Observe
entities, watch query results, and implement cache-first or stale-while-revalidate
patterns.
...
observeEntity(store, 'User', '1').subscribe((user) => {
  console.log(user);
  // { __typename: 'User', id: '1', name: 'Alice', ... } | undefined
});
```

### observeQuery

Emits a cached query result when any of its entity dependencies change. Extracts entity
references (`__typename` + `id`) from the cached data and listens for write/merge/evict
events on those entities.
...
Optionally pass a custom `extractData` function:
...
Emits cached data immediately if available, then fetches fresh data. The UI
renders instantly from cache while background-refreshing from the network.
...
Emits cached data immediately (stale), then fetches in background. Unlike
`cacheFirst`, fetch errors are silently swallowed when cached data exists.
...
Re-subscribes the source Observable when a watched entity changes. Useful for
automatically re-fetching data on mutation.
...
Fetches data, then observes the cached query result. Combines initial fetch
with `observeQuery` for automatic re-emission on entity changes.
...
Returns a Promise resolving to the cached query result, or `null`:
...
Returns an Observable that emits the current cached value and re-emits when
entity dependencies change:
...
Watches a single entity by `__typename` + `id`. Emits immediately, updates
on write/merge, emits `null` on evict:
